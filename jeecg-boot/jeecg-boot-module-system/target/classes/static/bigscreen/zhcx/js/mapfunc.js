        ...
        function (esriBasemaps, Map, Point, SpatialReference,
            ...
            layerObj.onClick = function (e) {
                ...
                for (var key in attributeNames) {
                	...
                	if (key == "PRJ_NAME") {
                		content += "<div id='customInfoDiv_title_" + layerObj.id + "' class='customInfoDiv_title'  ><font size=\"4\">" + e.graphic.attributes[attributeNames["PRJ_NAME"]] + "</font></div>";
                		continue;
                	}
                	if (e.graphic.attributes[attributeNames[key]] != null) {
                		p_name = e.graphic.attributes[attributeNames[key]];
                	}
                    content += key + "：" + p_name + "<br/>";
                }
...
var customInfoWindow_video = function (layerObj, title, attributeNames) {
    require(["esri/basemaps", "esri/map", "esri/geometry/Point", "esri/SpatialReference",
        "esri/layers/FeatureLayer",
        "esri/renderers/SimpleRenderer", "esri/symbols/PictureMarkerSymbol",
        "esri/geometry/ScreenPoint",
        ...
        function (esriBasemaps, Map, Point, SpatialReference,
            ...
            var drawCustomInfo = function (layerid, startPoint, endPointx, endPointy) {
                ...
                var polyline = new Polyline(polylineJson);
                var simpleLineSymbol = new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color('#000000'), 2);
                var graphicLineDrawn = new Graphic(polyline, simpleLineSymbol);
                map.getLayer("graphicsLayer_LineDrawn_" + layerid).add(graphicLineDrawn);
            };
            ...
            layerObj.onClick = function (e) {
                ...
                if ($('#customInfoDiv_' + layerObj.id).length > 0) {
                    ...
                    customInfoDiv = $("<div id='customInfoDiv_" + layerObj.id + "' class='customInfoDiv_o_video'>" +
                        "<div id='customInfoDiv_close_" + layerObj.id + "' class='customInfoDiv_close' title='关闭框体' onClick=\"$('#customInfoDiv_" + layerObj.id + "').hide();$('#customInfoDiv_iscustomInfoWindowshown_" + layerObj.id + "').text(0);map.getLayer('graphicsLayer_LineDrawn_" + layerObj.id + "').clear();\"><font size=\"5\">x</font></div>" +
                        
                        "<div id='customInfoDiv_content_" + layerObj.id + "' class='customInfoDiv_video'></div>" +
                        "<div id='customInfoDiv_footer_" + layerObj.id + "' class='customInfoDiv_foot_video'>" +
                        "<label id='customInfoDiv_scrPtx_" + layerObj.id + "' class='customInfoDiv_config'>" + e.graphic.geometry.x + "</label>" +
                        "<label id='customInfoDiv_scrPty_" + layerObj.id + "' class='customInfoDiv_config'>" + e.graphic.geometry.y + "</label>" +
                        "<label id='customInfoDiv_pointx_" + layerObj.id + "' class='customInfoDiv_config'>" + (scrPt.x + 10) + "</label>" +
                        "<label id='customInfoDiv_pointy_" + layerObj.id + "' class='customInfoDiv_config'>" + (scrPt.y - 150) + "</label>" +
                ...
                var content = "";
                for (var key in attributeNames) {
                	if (key == "BS_VIDEO_CHANNELNAME") {
                		content += "<div id='customInfoDiv_title_" + layerObj.id + "'  style=\"text-align:center;width:100%;height:6%;margin-top:5px\"  ><font size=\"4\">" + e.graphic.attributes[attributeNames["BS_VIDEO_CHANNELNAME"]] + "</font></div>";
                		
                	}
                	if (key == "BS_VIDEO_CHANNELID") {
                		content += "<div id='customInfoDiv_video_" + layerObj.id + "' style=\"margin-top:10px;width:100%;height:100%\">" +
                				"<iframe  id=\"frmroleadd\" name=\"frmroleadd\" src=\"VideoDemo2/demo.html?type="+title+"&channelid="+e.graphic.attributes[attributeNames["BS_VIDEO_CHANNELID"]]+"\" marginWidth=0 marginHeight=0 height=\"280px\" rameBorder=0 scrolling=\"no\"></iframe></div>";
...
//鼠标点击图层显示自定义的infowindow
var customInfoWindow_helmet = function (layerObj, title, attributeNames) {
  require(["esri/basemaps", "esri/map", "esri/geometry/Point", "esri/SpatialReference",
      "esri/layers/FeatureLayer",
      "esri/renderers/SimpleRenderer", "esri/symbols/PictureMarkerSymbol",
      "esri/geometry/ScreenPoint",
      ...
      function (esriBasemaps, Map, Point, SpatialReference,
          ...
          var drawCustomInfo = function (layerid, startPoint, endPointx, endPointy) {
              ...
              var polyline = new Polyline(polylineJson);
              var simpleLineSymbol = new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color('#676586'), 2);
              var graphicLineDrawn = new Graphic(polyline, simpleLineSymbol);
              map.getLayer("graphicsLayer_LineDrawn_" + layerid).add(graphicLineDrawn);
          };
          ...
          layerObj.onClick = function (e) {
              ...
              if ($('#customInfoDiv_' + layerObj.id).length > 0) {
                  ...
                  customInfoDiv = $("<div id='customInfoDiv_" + layerObj.id + "' class='customInfoDiv_helmet'>" +
                      "<div id='customInfoDiv_close_" + layerObj.id + "' class='customInfoDiv_close' title='关闭框体' onClick=\"$('#customInfoDiv_" + layerObj.id + "').hide();$('#customInfoDiv_iscustomInfoWindowshown_" + layerObj.id + "').text(0);map.getLayer('graphicsLayer_LineDrawn_" + layerObj.id + "').clear();\"><font size=\"5\">x</font></div>" +
                      "<div id='customInfoDiv_content_" + layerObj.id + "' class='customInfoDiv_content'></div>" +
                      "<div id='customInfoDiv_footer_" + layerObj.id + "' class='customInfoDiv_footer'>" +
                      "<label id='customInfoDiv_scrPtx_" + layerObj.id + "' class='customInfoDiv_config'>" + e.graphic.geometry.x + "</label>" +
