...
public class SysPermissionServiceImpl extends ServiceImpl<SysPermissionMapper, SysPermission> implements ISysPermissionService {
	...
	@Override
	public List<JSONObject> getProjectListPermission(String baseInfo, String userId	) {
/*		List<SysUserRole> sysUserRoleList = sysUserRoleMapper.selectList(new QueryWrapper<SysUserRole>().eq("USER_ID",userId));
		List<SysUserDepart> sysUserDepartList = sysUserDepartMapper.selectList(new QueryWrapper<SysUserDepart>().eq("USER_ID",userId));
		List<SysDepart> departList = sysUserDepartList.stream().map(item->{
			return sysDepartMapper.selectById(item.getDepId());
		}).collect(Collectors.toList());
		List<SysRole> roleList = sysUserRoleList.stream().map((item)->{
			return sysRoleMapper.selectById(item.getRoleId());
		}).collect(Collectors.toList());*/
		List<SysUserRole> sysUserRoleList = sysUserRoleMapper.selectList(new QueryWrapper<SysUserRole>().eq("USER_ID",userId));


		SysPermission parent = sysPermissionMapper.selectOne(new QueryWrapper<SysPermission>().eq("URL",baseInfo));
		List<SysPermission> children = sysPermissionMapper.selectList(new QueryWrapper<SysPermission>().eq("PARENT_ID",parent.getParentId()));
		children.sort(new Comparator<SysPermission>() {
			@Override
			public int compare(SysPermission sysPermission, SysPermission t1) {
				if(sysPermission.getSortNo()>t1.getSortNo())
					return 1;
				else return -1;
			}
		});
		...
		children.forEach(item->{
			...
			SysRolePermission sysRolePermission=sysRolePermissionMapper.selectOne(new QueryWrapper<SysRolePermission>().eq("ROLE_ID",sysUserRole.getRoleId()).eq("PERMISSION_ID",item.getId()));
			if (sysRolePermission!=null){
				JSONObject jsonObject = new JSONObject();
				jsonObject.put("path",item.getUrl());
				jsonObject.put("no",item.getSortNo());
				jsonObject.put("title",item.getName());
				String path = item.getUrl().substring(1).replaceAll("/","-");
				jsonObject.put("name",path);
				map.put(item.getUrl(),jsonObject);
