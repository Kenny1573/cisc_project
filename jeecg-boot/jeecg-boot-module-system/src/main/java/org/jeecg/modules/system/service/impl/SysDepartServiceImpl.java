...
public class SysDepartServiceImpl extends ServiceImpl<SysDepartMapper, SysDepart> implements ISysDepartService {
	...
	@Override
	public Result<?> queryDepartWithUser() {
		LambdaQueryWrapper<SysDepart> query = new LambdaQueryWrapper<SysDepart>();
		query.eq(SysDepart::getDelFlag, CommonConstant.DEL_FLAG_0.toString());
		query.orderByAsc(SysDepart::getDepartOrder);
		List<SysDepart> list = this.list(query);
		...
		return Result.ok(tranTree(listResult));
	}


	public JSONArray tranTree( List<SysDepartTreeModel> listResult){
		JSONArray res = new JSONArray();
		 listResult.forEach(item->{

			JSONObject jsonObject = new JSONObject();
			jsonObject.put("depCode",item.getOrgCode());
			jsonObject.put("depName",item.getDepartName());
			List<SysUser> listUsers = sysDepartMapper.queryUserByDepId(item.getId());
			JSONArray users= new JSONArray();
			listUsers.forEach(user->{
				JSONObject uobj=new JSONObject();
				uobj.put("realName",user.getRealname());
				uobj.put("userName",user.getUsername());
				users.add(uobj);
				});
			jsonObject.put("users",users);
			List<SysDepartTreeModel> children = item.getChildren();
			if (children!=null){
				jsonObject.put("children",tranTree(children));
