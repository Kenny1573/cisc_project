    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        <a-form-item label="年度" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-date-picker format="YYYY" mode="year" :allowClear="false"  :value="this.year" :open="open" @openChange="openChange" @panelChange="panelChange" @change="handleChange"></a-date-picker>
        </a-form-item>
...
<!--        <a-form-item label="目标财务数" :labelCol="labelCol" :wrapperCol="wrapperCol">
        ...
        <a-form-item label="形象进度" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="['ypImageProgress']" placeholder="请输入形象进度"></a-input>
        </a-form-item>
  ...
  export default {
    ...
    data () {
      return {
        ...
        validatorRules: {
          ypdYear: {
            rules: [
              { pattern: /^[0-9]\d{0,3}$/, message: '请输入数字!' }
            ]
          },
          ypdInvestment: {
            rules: [
          ...
          ypdPlan: {
            rules: [
              { pattern: /^.{0,100}$/, message:'超出最大输入长度，最大输入长度为100'},
            ]
          }
        },
        url: {
    ...
    methods: {
      ...
      edit (record) {
        ...
        if(this.year!=null) {
          this.model.ypYear = this.year.format('YYYY')
        }else{
          this.year=moment('2020-06','YYYY-MM')
          this.model.ypYear = this.year.format('YYYY')
        }
