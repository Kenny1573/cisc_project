  ...
  export default {
    ...
    methods: {
      ...
      sprySubmit() {
       ...
       // postAction(_this.url.editNodeUser,this.spry).then(res => {
        let params = {
              chooseDep:this.spry.chooseDep,
              chooseDepHeader: this.spry.chooseDepHeader,
              chooseDepHeader: this.spry.chooseDepHeader,
              departmentIds: this.spry.departmentIds,
              nodeId: this.spry.nodeId,
              roleIds: this.spry.roleIds,
              userIds: this.spry.userIds
            };

        postAction(_this.url.editNodeUser,params).then(res => {
          if (res.success) {
      ...
      viewForm(routeName) {
        let _this = this;
        //let routeName1='';
          routeName = _this.editObj.routeName;
        if (!routeName) {
        ...
        let formComponent = _this.getFormComponent(routeName);
        _this.lcModa.formComponent = formComponent.component;
        _this.lcModa.title = '流程表单预览：'+formComponent.text;
        _this.lcModa.visible = true;
      },
      convertToModel(row){
        let that = this;
        this.$confirm({
